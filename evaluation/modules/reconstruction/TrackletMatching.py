...
class TrackletMatcher:
    ...
    def loadTracklets(self, path):
        ...
        self.cam1Tracks = csv2Tracks(os.path.join(path, 'tracklets_2d_cam1.csv'),
        ...
        self.cam2Tracks = csv2Tracks(os.path.join(path, 'tracklets_2d_cam2.csv'),
        ...
        # /data/data0/liuyiran/Zef3d/eval_result/ZebraFish_05/tracklets_2d_cam1.csv
        cam1Info = "Camera 1\n\tLength: {}\n\tOffset: {}\n\tUnique IDs: {}".format(self.camera1_length,
                                                                                   self.camera1_offset,
                                                                                   len(self.cam1Tracks))
        cam2Info = "Camera 2\n\tLength: {}\n\tOffset: {}\n\tUnique IDs: {}".format(self.camera2_length,
                                                                                   self.camera2_offset,
                                                                                   len(self.cam2Tracks))
        print(cam1Info)
        print(cam2Info)
    ...
    def findConcurrent(self, track, candidates):
        """
        Finds the concurrent tracks (i.e. within the same span of frames) between a specific track and a set of
        other tracks
    ...
    def calcMatchWeight(self, track1, track2):
        ...
        The weight is a weighted median value of the inverse CDF value of the re-projection errors between the two tracks.
        The Gaussian CDF is used, with parameters loaded in the config file, and it is inverted so value below the mean
        (i.e. towards 0) is trusted more than value above
        
        Input:
            track1: Track object from the top camera
        ...
        if len(track3d.errors) > 0:
            ...
            weight = np.median(track3d.errors) * (
                        len(track3d.errors) / len(list(np.union1d(track1.frame, track2.frame))))
    ...
    def calcReprojError(self, frameNumber, track1, track2, verbose=False):
        ...
        if verbose:
            print(
                "Min error: {}\n\t3D coords: {}\n\tTrack 1: {}\n\tTrack 2: {}\n\tPos1 {} (GT) / {}\n\tPos2 {} (GT) / {}\n\tTrack 2 pt: {}".format(
                    minErr, minP, track1.id, track2.id, pos1, p1, pos2, p2, minPt))
    ...
    def createNodes(self, verbose=False):
        ...
        Also stores all the 3D tracklets in a internal triangulated structure
    ...
    def connectNodes3D(self, verbose=False):
        ...
        This is done if based on te average speed to travel between the two 2D tracklet positions of which is not the same tracklet
        ...
        for trackId in self.camIdMap:
            ...
            for e1 in elements:
                ...
                for e2 in elements:
                    ...
                    if verbose:
                        print("{}\n{}\n{}\n{}\n{}\n{}\n{}\n{}\n{}\n{}\n".format(e1, e2, e1Track.frame[0],
                                                                                e1Track.frame[-1], e2Track.frame[0],
                                                                                e2Track.frame[-1], overlap3D, overlap2D,
                                                                                frameDiff, posDiff))

                    ## If the tracklets start and ends frames differ too much, ignore it
                    if overlap3D or overlap2D or self.graph.has_edge(e1, e2) or self.graph.has_edge(e2,
                                                                                                    e1):  # Check that the tracklets does not temporally overlap, and that there is not already an edge in the DAG between the two tracklets
                        continue
                    ...
                    moveProb = (1.0 - scipy.stats.expon.cdf(speed, scale=self.movErrMean + self.movErrStd)) * np.exp(
                        -frameDiff / self.temporalPenalty)
...
def combine2DTracklets(df, tm):
    ...
    for iID in ids:
        ...
        for idx, sIdx in enumerate(dual_assignment.items()):
            ...
            if (rows.loc[frame_idx[0]]["cam1_x"] > -1.0 and rows.loc[frame_idx[1]]["cam2_x"] > -1.0) or (
                    rows.loc[frame_idx[0]]["cam2_x"] > -1.0 and rows.loc[frame_idx[1]]["cam1_x"] > -1.0):
                row_max = rows.max()
                ...
                # If the calculated point is within the aquarium, add it to the df, else do nothing
