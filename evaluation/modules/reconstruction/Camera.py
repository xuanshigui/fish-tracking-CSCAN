...
class Camera:
    ...
    def calcExtrinsic(self, worldPoints, cameraPoints, method=cv2.SOLVEPNP_ITERATIVE):
        ...
        Output:
            rvec: Rotation vector that together with tvec can transform from world to camera coordinates
            tvec: Translation vector that together with rvec can transform from world to camera coordinates
        """
...
class Plane:
    ...
    def intersectionWithRay(self, r, r0, verbose=False):
        """    
        Calculates the intersection between a plane and a ray
