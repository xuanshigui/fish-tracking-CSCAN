    ...
    def get_3d_tracks(self, tracks1, tracks2):
        ...
        if track_pairs is not None and len(track_pairs) != 0:
            for top, front in track_pairs.items():
                ...
                top_pos = top.pos
                # ToDo: Is this right?
                # key can't be the same, so if front=-1, it is actually top
                if front != -1 and front != -2:
                    ...
                    # key can't be the same, so if front=-1, it is actually top
                    # so cam1 is top view and is given the -1
                    track_3d_dict['id'] = top.id
                    track_3d_dict['err'] = -1
                    track_3d_dict['3d_x'] = -1
                    track_3d_dict['3d_y'] = -1
                    track_3d_dict['3d_z'] = -1
                    # center points
                    front_pos = -1
                    track_3d_dict['cam1_x'] = front_pos
                    track_3d_dict['cam1_y'] = front_pos
                    track_3d_dict['cam2_x'] = (top_pos[0] + top_pos[2]) / 2
                    track_3d_dict['cam2_y'] = (top_pos[1] + top_pos[3]) / 2
                    track_3d_dict['cam1_tl_x'] = front_pos
                    track_3d_dict['cam1_tl_y'] = front_pos
                    track_3d_dict['cam1_c_x'] = front_pos
                    track_3d_dict['cam1_c_y'] = front_pos
                    track_3d_dict['cam1_w'] = front_pos
                    track_3d_dict['cam1_h'] = front_pos
                    track_3d_dict['cam2_tl_x'] = top_pos[0]
                    track_3d_dict['cam2_tl_y'] = top_pos[1]
                    track_3d_dict['cam2_c_x'] = (top_pos[0] + top_pos[2]) / 2
                    track_3d_dict['cam2_c_y'] = (top_pos[1] + top_pos[3]) / 2
                    track_3d_dict['cam2_w'] = top_pos[2] - top_pos[0]
                    track_3d_dict['cam2_h'] = top_pos[3] - top_pos[1]
                elif front == -2:
                    # front=-2, front is the true front and it is none
                    track_3d_dict['id'] = top.id
                    front_pos = -1
                    track_3d_dict['err'] = front_pos
                    track_3d_dict['3d_x'] = front_pos
                    track_3d_dict['3d_y'] = front_pos
                    track_3d_dict['3d_z'] = front_pos
                    # center points
                    track_3d_dict['cam1_x'] = (top_pos[0] + top_pos[2]) / 2
                    track_3d_dict['cam1_y'] = (top_pos[1] + top_pos[3]) / 2
                    track_3d_dict['cam2_x'] = front_pos
                    track_3d_dict['cam2_y'] = front_pos
                    track_3d_dict['cam1_tl_x'] = top_pos[0]
                    track_3d_dict['cam1_tl_y'] = top_pos[1]
                    track_3d_dict['cam1_c_x'] = (top_pos[0] + top_pos[2]) / 2
                    track_3d_dict['cam1_c_y'] = (top_pos[1] + top_pos[3]) / 2
                    track_3d_dict['cam1_w'] = top_pos[2] - top_pos[0]
                    track_3d_dict['cam1_h'] = top_pos[3] - top_pos[1]
                    track_3d_dict['cam2_tl_x'] = front_pos
                    track_3d_dict['cam2_tl_y'] = front_pos
                    track_3d_dict['cam2_c_x'] = front_pos
                    track_3d_dict['cam2_c_y'] = front_pos
                    track_3d_dict['cam2_w'] = front_pos
                    track_3d_dict['cam2_h'] = front_pos

                tracks_3d.append(track_3d_dict)
        return tracks_3d

    def associate_tracks(self, tracks1, tracks2, pseudo_value=None):
        # Todo: process the vocation
        if len(tracks1) == len(tracks2):
            return self.associate_tracks_by_x_order(tracks1, tracks2)
        else:
            associations = self.associate_tracks_by_horizontal_distance(tracks1, tracks2)
            if len(associations) < len(tracks1):
                # Create pseudo associations for unassigned tracks in tracks2
                assigned_top = set(top for top, _ in associations.items())
                unassigned_top = [top for top in tracks2 if top not in assigned_top]
                if pseudo_value is None:
                    pseudo_value = -1  # Placeholder track with specific values
                for track2 in unassigned_top:
                    associations[track2] = pseudo_value
            elif len(associations) < len(tracks2):
                assigned_front = set(front for _, front in associations.items())
                unassigned_front = [front for front in tracks2 if front not in assigned_front]
                if pseudo_value is None:
                    pseudo_value = -2  # Placeholder track with specific values
                for track1 in unassigned_front:
                    associations[track1] = pseudo_value
            return associations
