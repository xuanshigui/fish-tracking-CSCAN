...
def create3DAnnotations(args):
    ...
    top["Frame"] = top["Frame"] + camera1_offset
    front["Frame"] = front["Frame"] + camera2_offset
    ...
    for frame in sorted(frames):
        for fish_ID in sorted(ids):
            ...
            if len(top_subdf) == 0:
                ...
                if err1 > 20.0:
                    print("Frame {} - cam frame {} - cam 1 - fish {} - reporj error {}".format(frame, top_subdf["Frame"].values[0], fish_ID, err1))
                    cam1_err.append("Frame {} - cam frame {} - cam 1 - fish {} - reporj error {}".format(frame, top_subdf["Frame"].values[0], fish_ID, err1))

                if err2 > 20.0:
                    print("Frame {} - cam frame {} - cam 2 - fish {} - reporj error {}".format(frame, front_subdf["Frame"].values[0], fish_ID, err2))
                    cam2_err.append("Frame {} - cam frame {} - cam 2 - fish {} - reporj error {}".format(frame, front_subdf["Frame"].values[0], fish_ID, err2))
                ...
                data_dict = {"frame":frame,
                            ...
                            "cam1_frame":[top_subdf["Frame"].values[0]],
                            "cam1_x": [top_subdf["X"].values[0]],
                            "cam1_y":[top_subdf["Y"].values[0]],
                            "cam1_tl_x": [top_subdf["Upper left corner X"].values[0]],
                            "cam1_tl_y": [top_subdf["Upper left corner Y"].values[0]],
                            ...
                            "cam1_br_y": [top_subdf["Lower right corner Y"].values[0]],
                            "cam1_occlusion": [top_subdf["Occlusion"].values[0]],
                            "cam2_frame": [front_subdf["Frame"].values[0]],
                            "cam2_x": [front_subdf["X"].values[0]],
                            "cam2_y": [front_subdf["Y"].values[0]],
                            "cam2_tl_x": [front_subdf["Upper left corner X"].values[0]],
                            "cam2_tl_y": [front_subdf["Upper left corner Y"].values[0]],
...
if __name__ == "__main__":
    ...
    ap.add_argument("-s1", "--syncTop", help="Sync frame for top camera", type=int, default=900)
    ap.add_argument("-s2", "--syncFront", help="Sync frame for front camera", type=int, default=900)
