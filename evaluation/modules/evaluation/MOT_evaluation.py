...
def MOT_Evaluation(args):
    ...
    if useMOTFormat:
        gt_df = pd.read_csv(gt_csv, sep=",", header=None, usecols=[0, 1, 2, 3, 4], names=['frame', 'id', '3d_x', '3d_y',
                                                                                          '3d_z'])
    else:
        gt_df = pd.read_csv(gt_csv, sep=",")
    det_df = pd.read_csv(det_csv, sep=",")
    # gt_df['id'] = [int(x) for x in gt_df['id']]
    ...
    if task == "3D":
        ...
        # gt_df = gt_df.dropna(axis='index', subset=[3, 4, 5])
        ...
        if bboxCenter:
            ...
            if "cam1_x" not in det_df.columns:
                det_df["cam1_x"] = det_df["cam1_c_x"]
                det_df["cam1_y"] = det_df["cam1_c_y"]
        ...
        if bboxCenter:    
            ...
            if "cam2_x" not in det_df.columns:
                det_df["cam2_x"] = det_df["cam2_proj_x"]
                det_df["cam2_y"] = det_df["cam2_proj_y"]
        ...
        gt_df["cam1_bb_tl_x"] = gt_df["cam1_left"]
        gt_df["cam1_bb_tl_y"] = gt_df["cam1_top"]
        gt_df["cam1_bb_br_x"] = gt_df["cam1_left"] + gt_df["cam1_width"]
        gt_df["cam1_bb_br_y"] = gt_df["cam1_top"] + gt_df["cam1_height"]
...
if __name__ == "__main__":
    ...
    ap.add_argument("-detCSV", "--detCSV", type=str, help="Path to tracking CSV file", default='/data/data0/liuyiran/Zef3d/others/mvi/ZebraFish-08.csv')
    ap.add_argument("-gtCSV", "--gtCSV", type=str, help="Path to ground truth CSV file", default='/data/data0/liuyiran/Zef3d/3DZeF20/test/ZebraFish-08/gt/gt.csv')

    ap.add_argument("-task", "--task", type=str, help="What kind of tracking to investigate [3D, cam1, cam2, cam1_bbox, cam2_bbox]", default='cam2')
    ...
    ap.add_argument("-thresh", "--thresh", type=float, help="Distance threshold", default=100)

    ap.add_argument("-outputFile", "--outputFile", type=str, help="Name of the output file", default="MOT_Metrics.txt")
    ap.add_argument("-outputPath", "--outputPath", type=str, help="Path to where the output file should be saved", default ="/data/data0/liuyiran/Zef3d/others/3d")
