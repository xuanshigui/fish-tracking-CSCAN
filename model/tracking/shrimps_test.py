...
import numpy as np
from datasets.shrimp_dataset_test import ShrimpsDataset_val
import csv
import os.path as osp
import yaml
from tracking.tracker_reid import Tracker
from tracking.deformable_detr_mod_reid import build as build_model
import argparse
...
def get_args_parser():
    ...
    # dataset parameters
    parser.add_argument('--dataset_file', default='fish')
    ...
    parser.add_argument('--trainval', action='store_true', default=True,
                        help='include validation in training and '
                             'test on test set')
    parser.add_argument('--half', default=True, action='store_true', help='half precision')
...
def main(tracktor):
    ...
    ds = ShrimpsDataset_val(root=main_args.data_dir, valset='test')
    ...
    models = [
        "/data/data0/liuyiran/transcenter/output/shrimps_enptqreid/val_best.pth",
    ]
    output_dirs = [
        curr_pth + '/test_models/shrimps_all_vb/',
    ]
    ...
    for model_dir, output_dir in zip(models, output_dirs):
        ...
        for idx, [samples, meta] in enumerate(data_loader):
            ...
            if video_name != pre_seq_name:
                ...
                tracker.inactive_patience = 80.0
