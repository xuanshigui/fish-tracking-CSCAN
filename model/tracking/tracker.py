...
class Tracker:
    ...
    def tracks_dets_matching_tracking(self, raw_dets, raw_scores, pre2cur_cts, pos=None, reid_cts=None,
        ...
        if raw_dets.nelement() > 0:
            ...
            if len(dets) > 0:
                ...
                # if len(u_track) > 0 and len(u_track) and len(self.tracks_T) == len(self.tracks_F):
                #     track_indices = copy.deepcopy(u_track)
                #     iou_dist = box_ops.generalized_box_iou(pos[u_track], dets[u_detection])
                #     iou_dist = 1 - iou_dist
                #     # todo recover inactive tracks here ?
                #     if view == 'top':
                #         matches_t, u_track_t, u_detection_t = self.linear_assignment(iou_dist.cpu().numpy(),
                #                                                                      thresh=0.9)
                #     else:
                #         matches_t, u_track_t, u_detection_t = self.linear_assignment(iou_dist.cpu().numpy(), thresh=0.1)
                #
                #     u_track = [track_indices[t_idx] for t_idx in u_track_t]
                #     if matches_t.shape[0] > 0:
                #
                #         for idx_track, idx_det in zip(matches_t[:, 0], matches_t[:, 1]):
                #             if view == 'top':
                #                 t = self.tracks_T[idx_track]
                #             else:
                #                 t = self.tracks_F[idx_track]
                ...
                #             t.add_features(det_feats[:, :, idx_det])
                #             t.score = scores_keep[[idx_det]]
                #     if len(u_detection_t) != 0:
                #         pos_birth = dets[u_detection_t, :]
                #         scores_birth = scores_keep[u_detection_t]
                #         dets_features_birth = det_feats[0, :, u_detection_t].transpose(0, 1)
                #     else:
                #         u_detection_t = []
                #         pos_birth = dets[u_detection_t, :]
                #         scores_birth = scores_keep[u_detection_t]
            ...
            if len(u_track) > 0:
                if len(dets_second) > 0:
                    ...
                    matches, u_track_second, u_detection_second = self.linear_assignment(iou_dist.cpu().numpy(),
                                                                                         thresh=0.5)  # stricter with low-score dets
    ...
    def step_reidV3_pre_tracking_vit(self, blob):
        ...
        # if len(self.tracks_T) != len(self.tracks_F):
        #     self.det_thresh = 0.5
        #     self.main_args.track_thresh = 0.3
        #     if len(self.tracks_T) == 0 or len(self.tracks_F) == 0:
        #         self.det_thresh = 0.1
        #         self.main_args.track_thresh = 0.1
        # else:
        #     self.det_thresh = 0.6
        #     self.main_args.track_thresh = 0.3
        #     if len(self.tracks_T) == 0 or len(self.tracks_F) == 0:
