...
from models.deformable_transformer_mod_enpntqcidpf import build_deforamble_transformer
import copy
...
class DeformableDETR(nn.Module):
    ...
    def __init__(self, transformer, num_classes, output_shape, half=True):
        ...
        self.nIDs = 360  # TODO: get this param from generic dataset
        ...
        self.reid_stem = nn.Sequential(nn.Linear(256, 64), nn.ReLU(inplace=True))

        self.reid = nn.Sequential(
            nn.Linear(64, self.nIDs), nn.ReLU(inplace=True)
        )
    ...
    def forward(self, samples: NestedTensor, pre_samples: NestedTensor, pre_cts: Tensor, track_query):
        ...
        for hs_m, pre_hs_m, re_hs_m in merged_hs:
            hs.append(hs_m)
            pre_hs.append(pre_hs_m)
            reid_hs.append(re_hs_m)
        ...
        for layer_lvl in range(len(hs)):
            ...
            reid_hs[layer_lvl] = self.reid_stem(reid_hs[layer_lvl])
