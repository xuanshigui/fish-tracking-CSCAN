...
import pandas as pd
import numpy as np
from datasets.fish_dataset_test import FishDataset_val
import csv
import os.path as osp
import yaml
from tracking.tracker_3d import Tracker
...
from tracking.matcher.TrackletMatching import TrackletMatcher
...
def get_args_parser():
    ...
    # dataset parameters
    parser.add_argument('--dataset_file', default='fish3df')
    parser.add_argument('--data_dir_T', default='/data/data0/liuyiran/dataset/3dzef_rev/imgT', type=str)
    parser.add_argument('--data_dir_F', default='/data/data0/liuyiran/dataset/3dzef_rev/imgF', type=str)
    ...
    parser.add_argument('--input_h', default=960, type=int)
    ...
    parser.add_argument('--track_thresh', type=float, default=0.3)  # default 0.3
    parser.add_argument('--new_thresh', type=float, default=0.3)  # default 0.3
    ...
    with open(file, "w") as of:
        ...
        for i, track in all_tracks.items():
            for frame, bb in track.items():
                ...
                x2 = bb[2]
                y2 = bb[3]
                writer.writerow([frame + frame_offset, i + 1, x1 + 1, y1 + 1, x2 - x1 + 1, y2 - y1 + 1, bb[4], -1, -1, -1])


def write_3d_results(all_tracks, out_dir, seq_name=None, frame_offset=0):
    output_dir = out_dir + "3d/"
    assert seq_name is not None, "[!] No seq_name, probably using combined database"
...
def main(tracktor):
    ...
    main_args.iou_recover_T = False
    main_args.iou_recover_F = False

    device = torch.device(main_args.device)
    ds_T = FishDataset_val(root=main_args.data_dir_T, valset='test')
    ds_F = FishDataset_val(root=main_args.data_dir_F, valset='test')

    ds_T.default_resolution[0], ds_T.default_resolution[1] = main_args.input_h, main_args.input_w
    ds_F.default_resolution[0], ds_F.default_resolution[1] = main_args.input_h, main_args.input_w
    ...
    main_args.match_thresh = 0.85
    model, criterion, postprocessors = build_model(main_args)
    # n_parameters = sum(p.numel() for p in model.parameters())
    # print('number of params:', n_parameters)

    # tracker
    tracker = Tracker(model, tracktor['tracker'], postprocessor=postprocessors['bbox'],
                      main_args=main_args)
    ...
    data_loader_T = DataLoader(ds_T, 1, shuffle=False, drop_last=False, num_workers=2, pin_memory=True,
                               collate_fn=collate_fn)
    data_loader_F = DataLoader(ds_F, 1, shuffle=False, drop_last=False, num_workers=2, pin_memory=True,
                               collate_fn=collate_fn)

    models = [
        "/data/data0/liuyiran/trans3d/output/fish3d_joint_ncga/checkpoint0029.pth",
        "/data/data0/liuyiran/trans3d/output/fish3d_joint_ncga/checkpoint0029.pth"
    ]
    output_dirs = [
        curr_pth + '/test_models/fish3d_top_ncgacauvb/',
        curr_pth + '/test_models/fish3d_front_ncgacauvb/'
    ]

    for model_dir, output_dir in zip(models, output_dirs):
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
    # load pretrained use the same configs now
    tracktor['obj_detect_model'] = models[0]
    tracktor['output_dir_T'] = output_dirs[0]
    tracktor['output_dir_F'] = output_dirs[1]
    print("Loading: ", tracktor['obj_detect_model'])
    model.load_state_dict(torch.load(tracktor['obj_detect_model'])["model"], strict=False)
    model.to(device)
    model.eval()
    ...
    start = 0
    first_frame = True
    tracker.match_3d = TrackletMatcher('./camera/' + 'ZebraFish_05')
    # samples is 0, mata is 1
    for idx, [image_T, image_F] in enumerate(zip(data_loader_T, data_loader_F)):
        num_frames += 1
        [orig_size, im_name, video_name, orig_img_T, trans] = image_T[1][0]
        [_, _, _, orig_img_F, _] = image_F[1][0]
        ...
        if video_name != pre_seq_name:
            ...
            if not os.path.exists(output_dir + "txt/" + pre_seq_name + '.txt') and idx != 0:
                ...
                print(f"Tracks found for top view: {len(results[0])}")
                print(f"Tracks found for front view: {len(results[1])}")
                print(f"Runtime for {pre_seq_name}: {time.time() - start :.2f} s.")
                write_results(results[0], tracktor['output_dir_T'], seq_name=pre_seq_name, frame_offset=frame_offset)
                write_results(results[1], tracktor['output_dir_F'], seq_name=pre_seq_name, frame_offset=frame_offset)
                write_3d_results(results[2], output_dir, seq_name=pre_seq_name, frame_offset=frame_offset)

            # update pre_seq_name #
            pre_seq_name = video_name
            first_frame = True

            pub_dets_T = ds_T.VidPubDet[video_name]
            pub_dets_F = ds_F.VidPubDet[video_name]

            # reset tracker #
            tracker.reset()
            tracker.match_3d = TrackletMatcher('./camera/' + video_name)
            # update inactive patience according to framerate

            tracker.inactive_patience = 60
        ...
        # starts with 0 #
        pub_det_T = pub_dets_T[int(im_name[:-4]) - 1]
        pub_det_F = pub_dets_F[int(im_name[:-4]) - 1]

        # print("step frame: ", im_name)

        batch = {'frame_name': im_name, 'video_name': video_name, 'imgT': orig_img_T.to(device),
                 'imgF': orig_img_F.to(device), 'samples_T': image_T[0][0].to(device),
                 'samples_F': image_F[0][0].to(device), 'orig_size': orig_size.unsqueeze(0).to(device),
                 'dets_T': torch.FloatTensor(pub_det_T)[:, :-1].to(device) if len(pub_det_T) > 0 else torch.zeros(0, 4),
                 'dets_F': torch.FloatTensor(pub_det_F)[:, :-1].to(device) if len(pub_det_F) > 0 else torch.zeros(0, 4),
                 'trans': trans}
    ...
    if not os.path.exists(output_dir + "txt/" + video_name + '.txt'):
        ...
        print(f"Tracks found for top view: {len(results[0])}")
        print(f"Tracks found for front view: {len(results[1])}")
        write_results(results[0], tracktor['output_dir_T'], seq_name=video_name, frame_offset=frame_offset)
        write_results(results[1], tracktor['output_dir_F'], seq_name=video_name, frame_offset=frame_offset)
        write_3d_results(results[2], output_dir, seq_name=pre_seq_name, frame_offset=frame_offset)
