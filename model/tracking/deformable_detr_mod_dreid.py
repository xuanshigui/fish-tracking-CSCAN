...
from models.deformable_transformer_mod_dreid import build_deforamble_transformer
import copy
...
class DeformableDETR(nn.Module):
    ...
    def __init__(self, transformer, num_classes, output_shape, half=True):
        ...
        self.nID = 360  # TODO: get this param from generic dataset
        ...
        self.reid = nn.Sequential(
            nn.Conv2d(64, 256, kernel_size=(3, 3), stride=(1, 1), padding=3 // 2, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv2d(256, self.nID, kernel_size=1, stride=1, padding=0, bias=True),
            nn.ReLU(inplace=True)
        )
        ...
        fill_fc_weights(self.reid)

        self.pre_memory = None
        self.masks_flatten = None
        self.pre_ref_pts = None

    def forward(self, samples: NestedTensor, pre_samples: NestedTensor, pre_cts: Tensor, track_query):
        ...
        with torch.no_grad():
            track_query = merged_hs[0][1].clone()
        hs = []
        pre_hs = []
        reid_hs = []
        ...
        for hs_m, pre_hs_m in merged_hs:
            ...
            reid_list = []
            for item in hs_m:
                reid_list.append(item.clone())
            reid_hs.append(reid_list)
        ...
        for layer_lvl in range(len(hs)):
            ...
            reid_hs[layer_lvl] = self.ida_up_reid(reid_hs[layer_lvl], 0, len(reid_hs[layer_lvl]))[-1]
            ...
            reid_head = self.reid(reid_hs[layer_lvl])
            # gather features #
            # (x,y) to index
