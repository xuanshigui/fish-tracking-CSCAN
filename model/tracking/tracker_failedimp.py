...
class Tracker:
    ...
    def __init__(self, obj_detect_T, obj_detect_F, tracker_cfg, postprocessor=None, main_args=None):

        self.obj_detect_T = obj_detect_T
        self.obj_detect_F = obj_detect_F
    ...
    def tracks_dets_matching_tracking(self, raw_dets, raw_scores, aug_dets, aug_score, pre2cur_cts, pos=None,
                                      reid_cts=None,
        ...
        # iou matching #
        if not (pos.nelement() > 0 and pos.shape[0] == pre2cur_cts.shape[0]):
            print(pos.nelement())
            print(pos.shape[0] == pre2cur_cts.shape[0])
        ...
        if raw_dets.nelement() > 0:
            ...
            if len(dets) > 0:
                ...
                if len(self.tracks_T) != len(self.tracks_F):
                    assigned = []
                    remove_inactive = []
                    if aug_dets.nelement() > 0:
                        aug_inds = aug_score > 0.5
                        remain_aug_inds = aug_dets[aug_inds]
                        # aug_ind_scores = raw_scores[aug_inds]
                        # agu_reid_cts = reid_cts[aug_inds]
                        if len(self.tracks_F) > len(self.tracks_T) and len(self.inactive_tracks_T) != 0:
                            inactive_pos = torch.cat([t.pos for t in self.inactive_tracks_T], dim=0)
                            iou_dist2 = box_ops.central_distance(inactive_pos, remain_aug_inds)
                            matches2, u_track2, u_detection2 = self.linear_assignment(iou_dist2.cpu().numpy(),
                                                                                      thresh=10)
                            if matches2.shape[0] > 0:
                                # update track dets, scores #
                                for r, c in zip(matches2[:, 0], matches2[:, 1]):
                                    t = self.inactive_tracks_T[r]
                                    t.count_inactive = 0
                                    t.pos = aug_dets[c].view(1, -1)
                                    t.reset_last_pos()
                                    t.add_features(det_feats[:, :, c])
                                    self.tracks_T.append(t)
                                    remove_inactive.append(t)
                            for t in remove_inactive:
                                self.inactive_tracks_T.remove(t)

                        if len(self.tracks_T) > len(self.tracks_F) and len(self.inactive_tracks_F) != 0:
                            inactive_pos = torch.cat([t.pos for t in self.inactive_tracks_F], dim=0)
                            iou_dist2 = box_ops.central_distance(inactive_pos, remain_aug_inds)
                            iou_dist2 = 1 - iou_dist2
                            matches2, u_track2, u_detection2 = self.linear_assignment(iou_dist2.cpu().numpy(),
                                                                                      thresh=0.95)
                            if matches2.shape[0] > 0:
                                # update track dets, scores #
                ...
                num = int(np.ceil((len(self.tracks_F) + len(self.tracks_T)) / 2))
                u_track = list(range(num))
                if view == 'top' and len(self.tracks_F) > len(self.tracks_T):
                    for t in self.inactive_tracks_T:
                        self.tracks_T.append(t)
                    self.inactive_tracks_T.clear()
                elif view == 'front' and len(self.tracks_T) > len(self.tracks_F):
    ...
    def detect_tracking_duel_vit(self, batch):
        ...
        if pos_F.shape[0] > 0:
            ...
            pre_cts_F = torch.zeros(size=(2, 2), device=pos_T.device, dtype=pos_T.dtype)
            no_pre_cts_F = True
        ...
        reid_cts_F[:, 0] /= outputs_T['reid'][0].shape[3]
        ...
        # filtered by scores #
        filtered_idx = labels_out_F == 1  # todo warning, wrong for multiple classes
        out_scores_T = out_scores_T[filtered_idx]
        out_scores_F = out_scores_F[filtered_idx]
        out_boxes_T = out_boxes_T[filtered_idx]
        out_boxes_F = out_boxes_F[filtered_idx]

        reid_cts_T = reid_cts_T[filtered_idx]
        reid_cts_F = reid_cts_F[filtered_idx]
    ...
    def step_reidV3_pre_tracking_vit(self, blob):
        ...
        if len(self.tracks_F):
            [det_pos_F, det_scores_F, dets_features_birth_F] = self.tracks_dets_matching_tracking(
                raw_dets=det_pos_F, raw_scores=det_scores_F, aug_dets=det_pos_T, aug_score=det_scores_T,
        ...
        for t in self.tracks_T:
            if t.id not in self.results_T.keys():
                self.results_T[t.id] = {}
            self.results_T[t.id][self.im_index] = np.concatenate([t.pos[0].cpu().numpy(), np.array([t.score.cpu()])])
        for t in self.tracks_F:
            if t.id not in self.results_F.keys():
                self.results_F[t.id] = {}
            self.results_F[t.id][self.im_index] = np.concatenate([t.pos[0].cpu().numpy(), np.array([t.score.cpu()])])
    ...
    def get_results(self):
        return self.results_T, self.results_F
