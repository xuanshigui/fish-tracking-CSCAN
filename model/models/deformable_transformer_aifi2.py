...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        for stage_idx in range(4):
            in_channels = d_model[stage_idx]
            input_att_list.append(
                nn.Sequential(AIFI(c1=in_channels), nn.Conv2d(in_channels, hidden_dim, kernel_size=1),
                              nn.BatchNorm2d(hidden_dim)))
        # query learning networks
        self.input_att = nn.ModuleList(input_att_list)
    ...
    def forward(self, src, pre_src=None, pre_cts=None, pre_memories=None,
        ...
        for stage in range(4):
            ...
            with torch.cuda.amp.autocast(self.half):
                if stage == 3:
                    attn = F.silu(self.input_att[stage](outs[stage]))
                    hs.append(self.input_proj[stage](outs[stage]) + attn)
                else:
