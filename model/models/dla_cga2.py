...
from models.CGA import CGAFusion
import torch
BN_MOMENTUM = 0.1
...
class IDAUpV3_bis(nn.Module):
    ...
    def __init__(self, o, channels, node_type=(DeformConv, DeformConv)):
        ...
        # self.up_2 = CARAFE(256, c_mid=512, scale=2, k_up=5, k_enc=3) # no params

        for i in range(0, len(channels)):
            c = channels[i]
            if i == 0:
                node = nn.Sequential(node_type[1](c, o), CGAFusion(dim=o))
            else:
                node = nn.Sequential(node_type[1](c, c), CGAFusion(dim=c))
    ...
    def forward(self, layers, startp, endp):
        for i in range(endp-1, startp, -1):
            ...
            res = layers[i-1]
            layers[i-1] = node(layers[i] + layers[i - 1]) + res
