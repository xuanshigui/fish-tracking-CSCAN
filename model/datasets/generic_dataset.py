...
class GenericDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        inp, padding_mask, anns_input, M, a, [ratio, padh, padw] = self._get_input(img, anns=copy.deepcopy(anns), flip=flipped)
        ret = {'image': inp, 'pad_mask': padding_mask.astype(np.bool_)}
        # ret['orig_image'] = img
        # print(img.shape)
        ...
        if opt.tracking:
            ...
            # flip image if flipped, reshape img to input size, get pad mask, if train do random affine,
            # return updated img, pad_mask and anns.
            # ret['pre_orig_image'] = pre_image
            pre_img, pre_padding_mask, pre_anns_input, pre_M, pre_a, _ = self._get_input(pre_image, anns=copy.deepcopy(pre_anns), flip=flipped, M=pre_M, a=pre_a)
            ...
            ret['pre_img'] = pre_img
            ret['pre_pad_mask'] = pre_padding_mask.astype(np.bool_)
        ...
        for k in range(num_objs):
            ...
            # bbox_amodal do not clip by output image size, bbox is clipped,
        ...
        track_ids_list = [key for key in curr_track_ids_cts]
        track_ids = np.zeros([self.max_objs])
        for idx, item in enumerate(track_ids_list):
            track_ids[idx] = item
        ret['track_ids'] = track_ids
        assert len(pre_cts) == len(pre_track_ids)
        ...
        #
        # img_pil.save(f"/coco/{img_info['id']:06}.png")
        # # mask_pil = Image.fromarray((padding_mask*255).astype(np.uint8))
        # # mask_pil.save(f"/coco/{img_info['id']:06}_pad.png")
        # # # #
        # # # # plot #
        #
        ...
        # img_pil.save(f"/coco/{img_info['id']:06}_{pre_img_id:06}_pre.png")
        ...
        # # plot hm #
        # hm_plot = ret['hm'][0].copy()
        # hm_plot = np.concatenate([hm_plot[:, :, np.newaxis], hm_plot[:, :, np.newaxis], hm_plot[:, :, np.newaxis]], axis=2)
        # hm_plot[:, :, 2] = 255 - hm_plot[:, :, 2] * 255
        # hm_plot[:, :, 1] = 0
        # hm_plot[:, :, 0] = hm_plot[:, :, 0] * 255
        # hm_plot = np.clip(hm_plot * 255, 0, 255)

        # hm_plot[hm_plot[:, :, 0] == 255, 1:2] *= 1
        #
        # # print(hm_plot.shape)
        # mask_pil = Image.fromarray(hm_plot.astype(np.uint32), mode='RGB')
        # mask_pil = mask_pil.resize((640, 480), Image.NEAREST)
        # mask_pil.save(f"/data/data0/liuyiran/transcenter/see_hm/{img_info['id']:06}_hm.jpg")
    ...
    def _add_instance(self, ret, k, cls_id, bbox, bbox_amodal, ann, curr_track_ids_cts):
