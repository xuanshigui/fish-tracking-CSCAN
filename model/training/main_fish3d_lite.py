...
def get_args_parser():
    ...
    parser.add_argument('--data_dir', default='fish', type=str)
    ...
    parser.add_argument('--resume', default='/data/data0/liuyiran/transcenter/model_zoo/pvtv2_backbone/pvt_v2_b0.pth', help='resume from checkpoint')
...
def main(args):
    ...
    t_input = torch.randn(1, 3, 960, 1280)
    t_input_mask = torch.zeros(1, 960, 1280)
    t_input = utils.NestedTensor(t_input, t_input_mask).to(device)
    t_pre_hm = torch.randn(1, 19, 2).to(device)
    flops, params = profile(model, inputs=(t_input, t_input, t_pre_hm))
    print('number of params:', n_parameters, 'Flops:', flops)
