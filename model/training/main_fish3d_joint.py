...
from training.deformable_detr_joint import build as build_model
from datasets.fish3df import Fish3dF
...
def get_args_parser():
    ...
    parser.add_argument('--pretrained', type=str,
                        default="/data/data0/liuyiran/trans3d/model_zoo/pvtv2_backbone/pvt_v2_b2.pth",
                        help="pretrained")
    ...
    parser.add_argument('--reid_weight', default=5.0, type=float)
    ...
    parser.add_argument('--data_dir_ch', default='crowd_human', type=str)
    ...
    # parser.add_argument('--resume', default='/data/data0/liuyiran/trans3d/output/fish3d_join_cga/checkpoint0024.pth', help='resume from checkpoint')
    ...
    parser.add_argument('--recover', action='store_true', help='recovery optimizer.')
    return parser
...
def main(args):
    ...
    dataset_train = Fish3dF(args, 'train')
    dataset_val = Fish3dF(args, 'test')
    ...
    # t_input_1 = torch.randn(1, 3, 1280, 960).to(device)
    # t_input_mask = torch.zeros(1, 960, 1280)
    # t_input_2 = torch.randn(1, 3, 1280, 960).to(device)
    # t_input_1 = utils.NestedTensor(t_input_1, t_input_mask).to(device)
    # t_input_2 = utils.NestedTensor(t_input_2, t_input_mask).to(device)
    # gt_instances = torch.randn(1, 20, 2).to(device)
    # flops, params = profile(model, inputs=(t_input_1, t_input_2, gt_instances, None))
    # print('——————————————————————————————————————NUM TRAINABLE MODEL PARAMS:', n_parameters, 'Flops:', flops)
